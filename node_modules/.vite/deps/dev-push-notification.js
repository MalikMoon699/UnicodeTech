...
async function getPushSubscription(swRegistration2, vapidKey) {
  const subscription = await swRegistration2.pushManager.getSubscription();
  if (subscription) {
    return subscription;
  }
  return swRegistration2.pushManager.subscribe({
...
async function updateSwReg(messaging2, swRegistration2) {
  if (!swRegistration2 && !messaging2.swRegistration) {
    await registerDefaultSw(messaging2);
  }
  if (!swRegistration2 && !!messaging2.swRegistration) {
    return;
  }
  if (!(swRegistration2 instanceof ServiceWorkerRegistration)) {
    throw ERROR_FACTORY2.create(
      "invalid-sw-registration"
      /* ErrorCode.INVALID_SW_REGISTRATION */
    );
  }
  messaging2.swRegistration = swRegistration2;
}
...
// node_modules/dev-push-notification/src/firebase.js
var messaging;
var initFirebase = (config) => {
  const appName = "push-sdk-app";
  let app;
...
// node_modules/dev-push-notification/src/push.js
var isInitialized = false;
var swRegistration = null;
var pendingListeners = [];
var BACKEND_URL = "https://dev-push-notification-backend.vercel.app";
var getToken3 = async (apiKey) => {
  try {
    ...
    const swRegistration2 = await navigator.serviceWorker.register(
      "/dpn-sw.js"
    );
    const token = await getToken2(messaging, {
      vapidKey,
      serviceWorkerRegistration: swRegistration2
    });
    if (!token) {
...
var onMessageListener = (callback, apiKey) => {
  if (isInitialized) {
...
var initMessaging = async (apiKey) => {
...
@firebase/messaging/dist/esm/index.esm2017.js:
   ...
   *)
*/
//# sourceMappingURL=dev-push-notification.js.map
